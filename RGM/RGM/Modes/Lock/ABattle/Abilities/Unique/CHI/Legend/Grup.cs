...
[Ability("GRU-P", """
                                    혼돈의 반란 단체, GRU-P 지원을 호출합니다.
                                    GRU-P 대원은 기본적으로 강화된 능력치를 가집니다.
                                    """, 
    AbilityCategory.Legend, AbilityType.LEGEND_CHI_GRUP, RoleAbility.CHI)]

public class Grup : Ability
{
    ...
    private static IEnumerator<float> SummonAlphaOneMembers()
    {
        ...
        foreach (Player target in targets)
        {
            ...
            if (target.IsAlive && target.Role.Type == RoleTypeId.ChaosRepressor)
                target.AddAbility(AbilityType.DUMMY_GRUPMENBER);
        }
    }
}